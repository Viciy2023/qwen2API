...
- [快速开始](#快速开始)
  ...
  - [方式三：Docker 本地构建](#方式三docker-本地构建)
  - [方式四：Zeabur 部署](#方式四zeabur-部署)
  - [方式五：Vercel 前端 + 独立后端](#方式五vercel-前端--独立后端)
...
此方式适用于开发者验证 Dockerfile、测试镜像构建链路或构建私有镜像。
...
- 此方式会在本地执行 Dockerfile 中的构建步骤。
- 构建过程中会下载前端依赖、后端依赖以及 Camoufox 浏览器内核。
- 如果服务器无法访问 Camoufox 下载源，不建议使用此方式作为生产部署方案。

---

### 方式四：Zeabur 部署

Zeabur 适合希望通过托管平台快速部署的场景。
...
2. 登录 Zeabur。
3. 新建项目并选择 GitHub 仓库。
4. 选择仓库后，Zeabur 会检测到 Dockerfile。
5. 在 Zeabur 的环境变量配置中设置至少以下变量：
   - `ADMIN_KEY`
   - `PORT=7860`
   - `ENGINE_MODE=hybrid`
6. 挂载持久化存储到 `/workspace/data` 和 `/workspace/logs`。
7. 部署完成后，通过 Zeabur 提供的域名访问服务。

#### 建议

- 确认平台支持较大的共享内存或浏览器运行环境。
- 如平台对无头浏览器支持较弱，建议优先使用 Docker 直拉镜像的云服务器部署方式。

---

### 方式五：Vercel 前端 + 独立后端

Vercel 仅适合作为前端托管平台，不适合作为后端运行平台。  
原因是：后端依赖浏览器引擎与持久化账号池，不适合部署在无状态 Serverless 环境。
...
- Vercel 页面仅负责访问前端静态资源。
- 所有 API 请求都通过 `VITE_API_BASE_URL` 转发到独立后端。
